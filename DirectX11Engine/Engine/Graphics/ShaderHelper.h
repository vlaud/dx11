...
static wstring GetShaderPath(VS_SHADER type)
{
	...
	case VS_SHADER::BUMP:
		path += L"vsBump.cso";
		break;
	case VS_SHADER::SPECULAR:
		path += L"vsSpecular.cso";
		break;
	}
	return path;
}
static wstring GetShaderPath(PS_SHADER type)
{
	...
	case PS_SHADER::BUMP:
		path += L"psBump.cso";
		break;
	case PS_SHADER::SPECULAR:
		path += L"psSpecular.cso";
		break;
	}
	return path;
}
...
D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0 });
		layout.push_back({ "NORMAL", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT,
D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0 });
		layout.push_back({ "TANGENT", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT,
D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0 });
		layout.push_back({ "BINORMAL", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT,
D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0 });
		break;
	}
}
