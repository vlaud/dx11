...
bool MeteorScene::SphereIntersect(unique_ptr<GameObject>& obj, XMVECTOR orgDir, XMVECTOR rayDir)
{
	XMFLOAT3 temp;
	...
	if (dist <= obj->GetComponent<SphereCollider>()->GetRadius())
	{
		//Picking
		return true;
	}
...
void MeteorScene::Picking()
{
	...
	if (SphereIntersect(model, orgDir, rayDir))
	{
		model.release();
	}
}
...
bool MeteorScene::IsPickingObs(unique_ptr<GameObject>& obj)
{
	...
	if (SphereIntersect(obj, orgDir, rayDir))
	{
		return true;
	}
	return false;
